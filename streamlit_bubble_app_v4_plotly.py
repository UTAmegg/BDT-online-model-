...
try:
    ...
    st.write("❌ plotly import failed:", str(e))
###########################################################################
...
def show_data_processing():
    """Data processing interface - CONVERTED TO PLOTLY"""
    ...
    # Show results if data is processed - CONVERTED TO PLOTLY
    if st.session_state.processed_data:
        ...
        fig = make_subplots(
            rows=1, cols=2,
            subplot_titles=('Original vs Interpolated Data', 'Interpolated R-t Curve'),
            horizontal_spacing=0.1
        )

        # Left plot: Original vs interpolated
        fig.add_trace(
            go.Scatter(
                x=st.session_state.t_original,
                y=st.session_state.R_original,
                mode='lines',
                name='Original Data',
        ...
        fig.add_trace(
            go.Scatter(
                x=st.session_state.t_interp_newunit,
                y=st.session_state.R_interp_newunit,
                mode='markers',
        ...
        # Right plot: Interpolated data only
        fig.add_trace(
            go.Scatter(
                x=st.session_state.t_interp_newunit,
                y=st.session_state.R_interp_newunit,
        ...
        # Update layout
        fig.update_xaxes(title_text="Time (0.1 ms)", row=1, col=1, showgrid=True, gridcolor='lightgray')
        fig.update_yaxes(title_text="Radius (0.1 mm)", row=1, col=1, showgrid=True, gridcolor='lightgray')
        fig.update_xaxes(title_text="Time (0.1 ms)", row=1, col=2, showgrid=True, gridcolor='lightgray')
        fig.update_yaxes(title_text="Radius (0.1 mm)", row=1, col=2, showgrid=True, gridcolor='lightgray')
...
def show_validation():
    """Validation interface - CONVERTED TO PLOTLY"""
    ...
    if st.button("🚀 Run Validation Simulation", type="primary"):
        ...
        with st.spinner("Running optimized bubble simulation..."):
            try:
                ...
                # Create comparison plot using Plotly - CONVERTED FROM MATPLOTLIB
                fig = go.Figure()
                ...
                fig.add_trace(
                    go.Scatter(
                        ...
                        y=st.session_state.R_interp_newunit,
                        mode='markers',
                        name='Interpolated (Experimental)',
                ...
                if len(t_sim) > 0 and len(st.session_state.t_interp_newunit) > 0:
                    ...
                    if t_max > t_min:
                        ...
                        if len(R_exp_common) > 0:
                            ...
                            error_text = f'RMSE: {rmse:.3f}<br>MAE: {mae:.3f}<br>Max Error: {max_error:.3f}'

                # Update layout
                fig.update_layout(
                    title='Validation: Experimental vs Simulated R-t Curves (Optimized)',
                    xaxis_title='Time (0.1 ms)',
                    yaxis_title='Radius (0.1 mm)',
                    showlegend=True,
                    plot_bgcolor='white',
                ...
                # Add error metrics as annotation if available
                if error_text:
                    fig.add_annotation(
                        x=0.02,
                        y=0.98,
